...
class plotWindow(QtGui.QMainWindow):
    ...
    def createMainFrame(self):
        ...
        for i in range(0, self.a[1]):  # a[0]-1
            ...
            self.colorLab.setStyleSheet(self.colorName(
                self.color[i]) + '; font-weight = bold;')
        ...
        self.connect(self.plotfuncbtn, QtCore.SIGNAL(
            'clicked()'), self.pushedPlotFunc)
        self.connect(self.multimeterbtn, QtCore.SIGNAL(
            'clicked()'), self.multiMeter)

        if self.plotType[0] == 0:
            self.analysisType.setText("<b>AC Analysis</b>")
            if self.plotType[1] == 1:
                self.connect(self.plotbtn, QtCore.SIGNAL(
                    'clicked()'), self.onPush_decade)
            else:
                self.connect(self.plotbtn, QtCore.SIGNAL(
                    'clicked()'), self.onPush_ac)

        elif self.plotType[0] == 1:
            self.analysisType.setText("<b>Transient Analysis</b>")
            self.connect(self.plotbtn, QtCore.SIGNAL(
                'clicked()'), self.onPush_trans)

        else:
            self.analysisType.setText("<b>DC Analysis</b>")
            self.connect(self.plotbtn, QtCore.SIGNAL(
                'clicked()'), self.onPush_dc)
    ...
    def pushedPlotFunc(self):
        ...
        if self.parts[1] == 'vs':
            if len(self.parts) > 3:
                ...
                self.axes.plot(self.combo, self.combo1,
                               c=self.color[1], label=str(2))  # _rev
            ...
            if self.plotType2[0] == 0:
                ...
                if self.plotType2[1] == 1:
                    ...
                    self.axes.plot(self.obj_dataext.x, finalResult,
                                   c=self.color[0], label=str(1))
