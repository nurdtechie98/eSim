...
class MainWindow(QtGui.QWidget):
    ...
    def callConvert(self):
        ...
        fw = open(os.path.join(projpath, project_name +
                               "_Previous_Values.json"), 'w')
        json_data = {}
        ...
        for line in store_schematicInfo:
            ...
            if words[len(words) - 1] == "ac":
                ...
                pulsed = {"Pulsed Value": str(
                    obj_source.entry_var[count].text())}
                count += 1
                json_data["source"][wordv]["values"].append(pulsed)

                rise = {"Rise Delay Time": str(
                    obj_source.entry_var[count].text())}
                count += 1
                json_data["source"][wordv]["values"].append(rise)
                ...
                fallConstant = {"Fall Time Constant": str(
                    obj_source.entry_var[count].text())}
                count += 1
        ...
        try:
            ...
            print("Netlist After Adding Device Model Library :",
                  store_schematicInfo)
    ...
    def createNetlistFile(self, store_schematicInfo, plotText):
        ...
        out.writelines('\n')
        sections = [simulatorOption, initialCondOption,
                    store_schematicInfo, analysisOption]
